...
function initPlayer() {
  ...
  var prevBtn      = el.querySelector('#mp-prev');
  var nextBtn      = el.querySelector('#mp-next');
  ...
  function sendCmd(cmd, value) {
    ...
    // Same-origin iframe: pin targetOrigin instead of '*' so we never leak
    // commands to a navigated/cross-origin document that briefly shares the frame.
    iframe.contentWindow.postMessage(
      { type: 'vm-cmd', cmd: cmd, value: value },
  ...
  window.addEventListener('message', function (e) {
    // Only accept messages from our own iframe and our own origin.
    ...
    if (e.data.type === 'vm-state') {
      ...
      vmTrack    = typeof e.data.trackName === 'string' ? e.data.trackName.slice(0, 200) : '';
      vmTime     = Number.isFinite(+e.data.currentTime) ? +e.data.currentTime : 0;
      vmDuration = Number.isFinite(+e.data.duration)    ? +e.data.duration    : 0;
      updateMiniUI();
    } else if (e.data.type === 'vm-freq') {
      // Expect a Uint8Array-like; reject anything else so we never feed
      // arbitrary objects into the EQ canvas loop.
      var d = e.data.data;
      if (d && (d instanceof Uint8Array || (typeof d.length === 'number' && d.length <= 4096))) {
        vmFreq = d;
      }
  ...
  /* ── minimize / restore ── */
  var minimizeBtn = el.querySelector('#mp-minimize');
  var MINIMIZED_KEY = 'portfolio-player-minimized';
  function setMinimized(min) {
    el.classList.toggle('minimized', !!min);
    try { sessionStorage.setItem(MINIMIZED_KEY, min ? '1' : '0'); } catch (e) {}
    if (minimizeBtn) {
      minimizeBtn.setAttribute('aria-label', min ? 'Restore player' : 'Minimize player');
      minimizeBtn.title = min ? 'Restore' : 'Minimize';
    }
    // Tell the iframe to throttle its EQ broadcast when minimized.
    sendCmd('idle', !!min);
  }
  if (minimizeBtn) {
    minimizeBtn.addEventListener('click', function (e) {
      e.stopPropagation();
      setMinimized(!el.classList.contains('minimized'));
    });
    minimizeBtn.addEventListener('mousedown', function (e) { e.stopPropagation(); });
    minimizeBtn.addEventListener('touchstart', function (e) { e.stopPropagation(); }, { passive: true });
  }
  // When minimized, a click anywhere on the puck restores the player
  el.addEventListener('click', function (e) {
    if (!el.classList.contains('minimized')) return;
    // Avoid firing when the click already came from a child control
    if (e.target.closest('button, a')) return;
    setMinimized(false);
  });
  try {
    if (sessionStorage.getItem(MINIMIZED_KEY) === '1') setMinimized(true);
  } catch (e) {}
